...
- (void)setCurrentFrame:(NSNumber*)pph {
    pixelsPerHour = pph;
    self.frame = CGRectMake((hoursOffset * [pixelsPerHour floatValue]) + kTLleftMargin, 0,([pixelsPerHour floatValue] * 24) - 3, self.superview.frame.size.height);
...
- (void)drawRect:(CGRect)rect
{
     ...
     */
    float pixelsForSixHours = [pixelsPerHour floatValue] * 6;
    ...
    for (int i = 1; i <= 3; i++) {
        ...
        if (i == 2) {
            ...
            showLine = ([pixelsPerHour floatValue] >= 40);
            CGContextSetLineWidth(ctx, 2.0f);
            
        }
        
        if (showLine) {
            CGContextMoveToPoint(ctx, i * pixelsForSixHours, startPoint);
            CGContextAddLineToPoint(ctx, i * pixelsForSixHours, self.frame.size.height);
            CGContextStrokePath(ctx);
